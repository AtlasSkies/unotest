...
function updateCharts() {
  ...
  [radar1, radar2].forEach(r => {
    if (!r) return;
    r.options.scales.r.suggestedMax = maxVal;
    const fill = computeFill(r, chartColor, axisColorPickers);
    r.data.datasets[0].data = vals;
    r.data.datasets[0].borderColor = chartColor;
    r.data.datasets[0].backgroundColor = fill;
    r.update();
  });
}
